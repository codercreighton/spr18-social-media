...
message_arr.each_with_index do |word, index|
  check = false
      if word[0] == "#"
        if word[word.length-1]=~/[!.,?]/ #regular expression check for punctuations to prevent duplicates
          check = true
          if Tag.pluck(:phrase).include?(word[0...word.length-1].downcase)
            tag = Tag.find_by(phrase: word[0...word.length-1].downcase)
          else
            tag = Tag.create(phrase: word[0...word.length-1].downcase)
          end 
